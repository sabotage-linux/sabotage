...
When finished compiling, you may do the following:

* Run `srcprep --rm-host-deps` to scan installed packages and remove
  all unneeded packages that were used only to build other packages.
  This is safe to run at any time to save space. At worst, packages
  will be reinstalled as-needed if you later add new packages.
* Run `srcprep --rm-build` to remove the development tools.
  You will not be able to compile and install new packages from
  source, however this will regain a considerable amount of space.
  This is something to do in the case of embedded use, typically.
* Run `srcprep --rm-ssl` to remove the SSL support. `butch` will
  lose the ability to download from HTTPS locations. If you know
  that HTTPS support is no longer needed in the chroot, this will
  save some space. Both `wpa_supplicant` and `xorg` depend upon
  having an SSL library to function.
* Run `srcprep --rm-fs-tools` to remove the filesystem utilities.
  This includes 'fsck', 'mkfs.ext4' and 'uuidgen'. If you are positive
  that you will not need these filesystem utilities, this will also
  save some space.
* Run `srcprep --rm-cxx-libs` to remove the GCC-provided C++ libraries.
  This will break a number of C++ applications, however it is likely
  you will not have any installed in a server or embedded situation.
* Run `srcprep --rm-src` when you are sure that you no longer need
  the Sabotage build system. This will save at least 10~12MB minimum
  plus anything that may be in `/src/build` and such.

If you follow all five options after completing stage1, it currently
results in a 12MB root that compresses to a 1.5MB .tar.xz.

You may run multiple use targets at once:

		srcprep --stage1 --rm-host-deps

For the most complete Sabotage experience only run `--rm-host-deps`.

After deciding what to remove, if anything, exit the chroot and either:
